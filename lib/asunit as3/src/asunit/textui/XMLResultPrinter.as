    ...
    **/
    public class XMLResultPrinter extends ResultPrinter {
        ...
        override public function printResult(result:TestResult, runTime:Number):void {
            ...
            FlashConnect.trace("<XMLResultPrinter>");
            FlashConnect.trace("<?xml version='1.0' encoding='UTF-8'?>");
            FlashConnect.trace("<testsuites>");
            FlashConnect.trace("<testsuite name='AllTests' errors='" + result.errorCount() + "' failures='" + result.failureCount() + "' tests='" + result.runCount() + "' time='" + elapsedTimeAsString(runTime) + "'>");
            var xmlTestResult:XMLTestResult;
            for each(xmlTestResult in results) {
                FlashConnect.trace(xmlTestResult.toString());
            }
            FlashConnect.trace("</testsuite>");
            FlashConnect.trace("</testsuites>");
            FlashConnect.trace("</XMLResultPrinter>");
        }
    }
